...
class FirstNameTests(TestCase):
    ...
    def test_create__when_first_name_does_not_contain_only_letters__expect_to_raise(self):
        with self.assertRaises(ValidationError) as ve:
            AccountProfile.objects.create(**self.invalid_account_profile_data).full_clean()

        error_messages = ve.exception.message_dict
        actual_error_message = error_messages.get('first_name')[0]

        self.assertEqual(AccountProfile.ONLY_LETTERS_EXCEPTION_MESSAGE, actual_error_message)
