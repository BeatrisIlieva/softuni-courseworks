...
from e_commerce_website.jewelry.style_funcs import define_jewelries_count_after_selected_style
...
def define_jewelries_count_before_selected_stone_type(stone_type_ids, jewelries, jewelries_count_by_stone_type):
    for index, stone_type_id in enumerate(stone_type_ids):
        id_for_label = f'id_stone_type_choices_{index}'
        jewelries_count_by_stone_type[id_for_label] = jewelries.prefetch_related('jewelry_stones__stone_type').filter(
            jewelry_stones__stone_type_id__exact=stone_type_id).count()
...
def define_fields_by_stone_type_choice(selection_pattern_stone_types, jewelries, jewelries_count_by_style):
    ...
    jewelries_count_by_style = \
        define_jewelries_count_after_selected_style(jewelries, style_ids, jewelries_count_by_style)
