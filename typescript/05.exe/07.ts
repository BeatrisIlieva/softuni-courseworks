    ...
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = function () {
            ...
            const latest = data.filter(
                (x: any) =>
                    currentDate.getTime() - x.createdOn.getTime() <= seconds * 1000
            );
...
function Censor<T extends { [key: string]: any }>(censorService: MockCensorService<T>) {
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        const originalMethod = descriptor.value;

        descriptor.value = function () {
            const data = originalMethod.call(this);
            const censoredData = censorService.censorProperties(data);
