...
class VideoGame(models.Model):
    ...
    rating = models.DecimalField(
        max_digits=2, decimal_places=1, validators=[validate_rating]
    )
...
class Invoice(models.Model):
    ...
    def get_invoice_with_billing_info(cls, invoice_number: str):
        return cls.objects.select_related("billing_info").get(
...
class Task(models.Model):
    ...
    def ongoing_high_priority_tasks(self):
        return self.objects.filter(
            priority="High", is_completed=False, completion_date__gt=F("creation_date")
        )

    def completed_mid_priority_tasks(self):
        return self.objects.filter(priority="Medium", is_completed=True)
...
class Exercise(models.Model):
    ...
    def get_exercises_within_duration(cls, min_duration: int, max_duration: int):
        return cls.objects.filter(
            duration_minutes__gte=min_duration, duration_minutes__lte=max_duration
        )
    ...
    def get_exercises_with_difficulty_and_repetitions(
        ...
        return cls.objects.filter(
            difficulty_level__gte=min_difficulty, repetitions__gte=min_repetitions
        )
