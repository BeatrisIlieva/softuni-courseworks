...
function CensorUsers<T extends { [key: string]: any }>(
    censorService: MockCensorService<T>
) {
    let lastCreated: Date | null;
    let cache: T[];
    ...
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        ...
        if (lastCreated && cache && !(currentTime.getTime() - lastCreated.getTime() > 5000)) {
            descriptor.value = function () {
                const censoredItems = censorService.censorProperties(cache);
        ...
        descriptor.value = function () {
            ...
            const censoredItems = censorService.censorProperties(items);
...
function CensorEmployees<T extends { [key: string]: any }>(
    censorService: MockCensorService<T>
) {
    let lastCreated: Date | null;
    let cache: T[] | null;
    ...
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        ...
        if (lastCreated && cache && !(currentTime.getTime() - lastCreated.getTime() > 10000)) {
...
class UsersService {
    ...
    @CensorUsers(userCensorService)
    getUsers() {
        return this._users;
    }

    @CensorEmployees(employeeCensorService)
    getEmployees() {
